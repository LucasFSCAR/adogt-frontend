...
import com.example.myapplication.ui.activities.PetEditActivity
...
class PetEditListAdapter(private var petList: List<Pet>) :
        RecyclerView.Adapter<PetEditListAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        val status = petList[position].status


        val petCard = holder.view.findViewById<MaterialCardView>(R.id.pet_card)
        petCard.setOnClickListener{
            val i = Intent(holder.view.context, PetEditActivity::class.java)
