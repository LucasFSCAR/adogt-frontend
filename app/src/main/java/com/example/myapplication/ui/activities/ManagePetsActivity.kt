...
import com.example.myapplication.ui.adapters.PetEditListAdapter
...
class ManagePetsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewAdapter = PetEditListAdapter(emptyList())
        ...
        call.enqueue(object : Callback<List<Pet>> {
            override fun onResponse(call: Call<List<Pet>>?, response: Response<List<Pet>>?) {
                if (response != null && response!!.body() != null) {
                    response!!.body()?.let {
                        (viewAdapter as PetEditListAdapter).updateData(it)
                        if(it.size == 0)
